$(document).ready(function () {
    ...
    function getBoosts(startIndex, max, scrollToTop, old, shouldPew) {
        ...
        var url = '/api/v1/sent?index=' + boostIndex;
        if (max > 0) {
            url += '&count=' + max;
        }
        if (old) {
            ...
            success: function (data) {
                ...
                if ($('div.outgoing_msg').length == 0 && $('div.nodata').length == 0) {
                    inbox.prepend('<div class="nodata"><p>No data to show yet. Building the initial database may take some time if you have many ' +
                        'transactions, or maybe you have not sent any satoshis yet?</p>' +
                        '<p>This screen will automatically refresh as you send satoshis.</p>' +
                        '<p><a href="https://podcastindex.org/apps">Check out a Podcasting 2.0 app to send satoshis.</a></p>' +
                ...
                if ($('div.outgoing_msg').length > 0 && $('div.loadmore').length == 0 && (boostIndex > 1 || noIndex)) {
                    inbox.append('<div class="loadmore"><a href="#">Show older sent boosts...</a></div>');
                }
            }
        });
    }
    ...
    //Get the most recent payment index the node knows about
    function getIndex() {
        //Get the current payment index number
        $.ajax({
            url: "/api/v1/sent_index",
            type: "GET",
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function (data) {
    ...
    //Render the sent boost info modal
    function renderSentInfo() {
          ...
          <div class="modal-dialog modal-lg modal-dialog-centered">
            <div class="modal-content">
              <div class="modal-header">
                <h5 class="modal-title">Sent Boost</h5>
        ...
        $dialog.on('show.bs.modal', function (ev) {
            ...
            $table.html('Loading sent boost...');

            $.getJSON(`/api/v1/sent?index=${msgid}&count=1&old=true`, (result) => {
                if (!result[0]) {
                    return $table.html('Sent boost not found!');
                }
